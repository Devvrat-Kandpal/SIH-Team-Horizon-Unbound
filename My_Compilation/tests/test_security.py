...
def test_operator_endpoint_with_valid_api_key_header(client):
    ...
    resp = client.post("/api/inject-fault", json={"event_type": "THERMAL_DRIFT"}, headers=headers)
...
def test_operator_endpoint_with_valid_bearer_token(client):
    ...
    resp = client.post("/api/set-criticality", json={"criticality_level": 3}, headers=headers)
    assert resp.status_code == 200
...
def test_operator_endpoint_with_invalid_key_rejected(client):
    ...
    resp = client.post("/api/inject-fault", json={"event_type": "ELECTRICAL_SPIKE"}, headers=headers)
